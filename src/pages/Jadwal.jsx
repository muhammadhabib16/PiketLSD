...
export default function Jadwal() {
  ...
  return (
    <div className="max-w-5xl mx-auto w-full space-y-5 sm:space-y-6 pb-6 animate-fadeIn">
      {/* 1. Header Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3.5 sm:pb-4 border-b border-slate-200">
        ...
        <button
          ...
          className="self-start sm:self-auto px-4 py-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 active:scale-95 text-xs font-bold text-slate-700 shadow-xs flex items-center gap-2 transition disabled:opacity-50 touch-manipulation"
        >
      ...
      <div className="rounded-3xl p-4 sm:p-5 bg-blue-600 text-white shadow-sm border border-blue-500/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-white border border-slate-200/90 text-xs sm:text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition shadow-xs"
            />
          </div>

          {/* Quick Day Selector Tabs with safe edge padding */}
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 no-scrollbar pe-4">
            <button
              onClick={() => setSelectedDay('ALL')}
              className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition shadow-xs active:scale-95 touch-manipulation min-h-[38px] ${
                selectedDay === 'ALL'
            ...
            {daysList.map((day) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition flex items-center gap-1.5 shadow-xs active:scale-95 touch-manipulation min-h-[38px] ${
                    selectedDay === day
                      ? 'bg-blue-600 text-white shadow-blue-500/20'
                      : isToday
      ...
      {filteredJadwal.length === 0 ? (
        <div className="p-10 text-center rounded-3xl bg-white border border-slate-200 shadow-xs space-y-2.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-4 lg:gap-5">
          {filteredJadwal.map((item) => {
            ...
            return (
              <div
                key={item.hari}
                className={`rounded-3xl p-4 sm:p-5 bg-white border transition-all shadow-xs hover:shadow-md flex flex-col justify-between ${
                  isToday
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between pb-2.5 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                      ...
                      {isToday && (
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-950 border border-amber-300 flex items-center gap-1 shadow-2xs">
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center space-x-2.5 min-w-0">
                              <div
                                className={`w-8 h-8 rounded-xl flex items-center justify-center text-xs font-extrabold flex-shrink-0 shadow-2xs ${
                                  isUser
                            ...
                            <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-lg bg-white border border-slate-200 text-slate-700 flex-shrink-0">
                ...
                {/* Footer Info */}
                <div className="mt-3.5 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[10.5px] text-slate-400 font-medium">

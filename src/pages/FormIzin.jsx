...
export default function FormIzin() {
  ...
  return (
    <div className="max-w-2xl mx-auto w-full space-y-4 pb-20 animate-fadeIn">
      {/* 1. Header Title */}
      ...
      {successInfo ? (
        <div className="bg-white rounded-2xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-5 animate-fadeIn">
          ...
          <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-2.5 text-xs md:text-sm">
            <div className="flex items-center justify-between py-1 border-b border-slate-200">
              <span className="text-slate-500 flex items-center gap-1.5">
                <Calendar className="w-4 h-4 text-rose-500" /> Tanggal Berhalangan:
              </span>
            ...
            <div className="flex items-center justify-between py-1 border-b border-slate-200">
              <span className="text-slate-500 flex items-center gap-1.5">
                <CalendarDays className="w-4 h-4 text-emerald-600" /> Tanggal Pengganti:
              </span>
            ...
            <div className="pt-1.5 space-y-1">
              <span className="text-slate-500 flex items-center gap-1.5">
                <FileText className="w-4 h-4 text-blue-600" /> Alasan:
              </span>
          ...
          <div className="flex gap-3 pt-1">
            <button
              onClick={() => navigate('/')}
              className="flex-1 py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs md:text-sm font-bold transition active:scale-95 shadow-sm"
            >
              Kembali ke Beranda
            </button>
            <button
              onClick={handleReset}
              className="flex-1 py-2.5 px-4 rounded-xl bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 text-xs md:text-sm font-bold transition active:scale-95 shadow-sm"
            >
              Buat Pengajuan Baru
            </button>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {errorMessage && (
            <div className="p-3.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-900 text-xs md:text-sm flex items-start gap-2.5 animate-fadeIn">
          ...
          <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  disabled={submitting}
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-xs md:text-sm text-slate-900 focus:outline-none focus:border-blue-600 focus:bg-white transition"
                  required
                />
              </div>
            ...
            <div className="space-y-1.5">
              <label className="block text-xs md:text-sm font-bold text-slate-800">
                Alasan Berhalangan / Keterangan Tukar Jadwal *
              </label>
              <textarea
                rows={3}
                value={alasan}
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-xl p-3 text-xs md:text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-blue-600 focus:bg-white transition resize-none"
                required
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-3.5 px-5 rounded-xl bg-blue-600 hover:bg-blue-700 active:scale-98 text-white font-bold text-xs md:text-sm flex items-center justify-center space-x-2 transition shadow-md disabled:opacity-60"
          >
            {submitting ? (
              <>
